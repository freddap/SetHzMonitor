...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        editConfigItem.Click += (s, e) =>
        {
            string configFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "config", "ProcessHzMap.cfg");
            ...
            {
                ShowTopmostMessageBox("Config file not found.");
            }
        };
        ...
        restartItem.Click += (s, e) =>
        {
            ...
            int newHz = processHzMap.ContainsKey("default") ? processHzMap["default"] : 0;

            if (!SetRefreshRate(newHz))
            {
                form.BeginInvoke(new Action(() =>
                {
                    ShowTopmostMessageBox($"Failed to set refresh rate to {newHz} after restart");
                }));
            }
            else
            {
                ...
                form.BeginInvoke(new Action(() =>
                {
                    var popup = new NeonPopup($"{currentHz} Hz");
                    popup.Show();
                }));
            }
        };
        ...
        Task.Run(() =>
        {
            while (running)
            {
                ...
                if (currentHz != targetHz)
                {
                    ...
                    {
                        currentHz = targetHz; // UPDATE CURRENTHZ BEFORE
